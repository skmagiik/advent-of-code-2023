...
fn main() {
    ...
    let mut seconds = time.elapsed().unwrap().as_secs();
    let hours = seconds / 3600;
    seconds = seconds - (hours*3600);
    let minutes = seconds / (60);
    seconds = seconds - (minutes*60);
    println!("{:02}:{:02}:{:02} elapsed", hours, minutes, seconds);
}
...
fn part1(input: &str) -> u32 {
    ...
    let seed_range = match parse_seeds(sections[0]) {
        Ok((_, s)) => s,
        _ => Vec::new()
    };
    ...
    for i in (0..seed_range.len()/2){
        println!("{}..{}", seed_range[i*2], seed_range[i*2]+seed_range[i*2+1]);
        for j in (seed_range[i*2]..seed_range[i*2]+seed_range[i*2+1]){
            seeds.push(j);
        }    
    }
    println!("{} seeds generated", seeds.len());
    // println!("seeds: {:?}", seeds);

    let mut seed_val: usize = 0;
    let mut seed_max = seeds.len();
    println!("");
    for seed in seeds {
        let mut cur_seed = seed;
        seed_val += 1;
        print!("\rmapping seed: {:.2}%", ((seed_val*100) as f32 / seed_max as f32));
